...
fn test_bb_to_cw() -> Result<()> {
    ...
    let bb_to_cw = BBToCWBuilder::new().accuracy(100).samples_per_dot(samples_per_dot).build();
    ...
    println!("CW-Alphabet Vector Length: {}, Content: {:?}", received.len(), received);
    assert_eq!(&vec![Dot, Dot, Dot, WordSpace, Dash, Dash, Dash, LetterSpace, WordSpace, Dot, Dot, Dot], received);
